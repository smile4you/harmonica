...
function getScaleNoteNames(rootNote, scaleType) {
    ...
    let rootS = (_a = baseS[rootLetter]) !== null && _a !== void 0 ? _a : 0;
    for (const ch of rootAcc) {
        if (ch === '♯')
            rootS++;
        else if (ch === '♭')
            rootS--;
    ...
    return pattern.map((offset, i) => {
        ...
        let diff = target - ((_a = baseS[letter]) !== null && _a !== void 0 ? _a : 0);
        if (diff > 6)
            diff -= 12;
        if (diff < -6)
            diff += 12;
...
function getDiatonicChordQuality(scaleNotes, degree) {
    const baseS = { C: 0, D: 2, E: 4, F: 5, G: 7, A: 9, B: 11 };
    const toS = (name) => {
        var _a;
        let s = (_a = baseS[name[0]]) !== null && _a !== void 0 ? _a : 0;
    ...
    const iv = (idx) => ((toS(scaleNotes[(degree + idx) % 7]) - rootS) + 12) % 12;
...
class HarmonicaUI {
    constructor() {
        ...
        this.pianoOctaveFilter = null; // null = all octaves
    ...
    setupEventListeners() {
        ...
        const pianoOctaveSelect = document.getElementById('pianoOctaveSelect');
        ...
        if (noteLengthSlider) {
            ...
            if (noteLengthDisplay)
                noteLengthDisplay.textContent = `${this.noteDuration.toFixed(1)}s`;
    ...
    updateSheetMusic() {
        ...
        const staffPos = (noteName) => {
            var _a;
            const letter = noteName[0];
            const octave = parseInt(noteName.replace(/[^0-9]/g, ''));
            return (octave - 4) * 7 + ((_a = letterIndex[letter]) !== null && _a !== void 0 ? _a : 0) - 2; // 0 = E4
        };
        const noteY = (sp) => bottomLineY - sp * (LS / 2);
        const parts = [];
        // Staff lines
        for (let i = 0; i <= 4; i++) {
    ...
    loadSettings() {
        ...
        try {
            ...
            if (savedSettings) {
                ...
                this.pianoOctaveFilter = (_c = settings.pianoOctaveFilter) !== null && _c !== void 0 ? _c : null;
                this.currentKey = (_d = settings.currentKey) !== null && _d !== void 0 ? _d : 'C';
                this.currentPosition = (_e = settings.currentPosition) !== null && _e !== void 0 ? _e : 1;
                this.currentScale = (_f = settings.currentScale) !== null && _f !== void 0 ? _f : 'chromatic';
                this.noteDuration = (_g = settings.noteDuration) !== null && _g !== void 0 ? _g : 1.0;
            }
        }
        catch (error) {
    ...
    getScaleNotes() {
        ...
        return harmonicaLayout
            ...
            if ((n.bend === 'ob' || n.bend === 'od') && !this.showOverblows)
                return false;
            const { inScale } = getNoteInterval(n.note, rootNote, this.currentScale);
            if (!inScale || this.currentScale === 'chromatic')
                return false;
            const octaveMatch = this.pianoOctaveFilter === null ||
    ...
    highlightPlayingNote(noteId) {
        ...
        if (noteId) {
            ...
            if (oval)
                oval.setAttribute('fill', 'rgb(255, 215, 0)');
        }
    }
    updatePlayButton() {
