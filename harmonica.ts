...
const enharmonicDisplayPairs: { [key: string]: [string, string] } = {
    'C♯': ['C♯', 'D♭'], 'D♭': ['C♯', 'D♭'],
    'D♯': ['D♯', 'E♭'], 'E♭': ['D♯', 'E♭'],
    'F♯': ['F♯', 'G♭'], 'G♭': ['F♯', 'G♭'],
    'G♯': ['G♯', 'A♭'], 'A♭': ['G♯', 'A♭'],
...
function getDisplayNoteName(noteName: string): string {
...
function computeNoteFrequency(noteName: string): number {
    ...
    const semisFromC: { [key: string]: number } = {
...
class HarmonicaUI {
    ...
    private droneNoiseSource: AudioBufferSourceNode | null = null;
    ...
    private initializeHarmonica(): void {
        ...
        holes.forEach(holeNum => {
            ...
            const isOverblow = (n: HarmonicaNote) => n.bend === 'ob' || n.bend === 'od';
            ...
            if (blowNotes.length > 0) {
                ...
                const sortedBlowNotes = [...blowNotes].sort((a, b) => {
                    if (a.bend && !b.bend) return -1; // bend/overblow before main
                    if (!a.bend && b.bend) return 1;  // main after bend/overblow
            ...
            if (drawNotes.length > 0) {
                ...
                const sortedDrawNotes = [...drawNotes].sort((a, b) => {
                    if (!a.bend && b.bend) return -1; // main before bend/overdraw
                    if (a.bend && !b.bend) return 1;  // bend/overdraw after main
                    return b.frequency - a.frequency;  // within same type: desc frequency
                });
    ...
    private updatePianoLabels(): void {
        ...
        document.querySelectorAll<HTMLElement>('.piano-key').forEach(key => {
            ...
            const keyFreq = 440 * Math.pow(2, ((octave - 4) * 12 + (semitonesFromC[note] ?? 0) - 9) / 12);
    ...
    private setupEventListeners(): void {
        ...
        const overblowToggle = document.getElementById('overblowToggle') as HTMLInputElement;
        if (overblowToggle) {
            // Restore saved state
        ...
        if (droneBtn) droneBtn.addEventListener('click', () => {
            if (this.isDroning) this.stopDrone(); else this.startDrone();
        });
    ...
    private stopDrone(): void {
        ...
        if (ctx && masterGain) {
            ...
            if (noise) noise.stop(now + 0.3);
        }
    }

    private updateDroneButton(): void {
        const btn = document.getElementById('droneBtn') as HTMLButtonElement | null;
        if (btn) btn.textContent = this.isDroning ? '■ Stop Drone' : '♩ Drone';
    }

    private restartDroneIfActive(): void {
        if (!this.isDroning) return;
        this.stopDrone();
